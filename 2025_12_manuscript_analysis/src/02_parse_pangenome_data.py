...
roary_file_path = os.path.join(input_path, 'core_accessory_graph.dot')
...
# Roary
def roary_dot_to_igraph(dot_file):
    ...
    weight_pattern = re.compile(r'weight\s*=\s*"([^"]+)"')

    def clean_dot_label(label):
    ...
    total_lines = sum(1 for line in open(dot_file, 'r'))

    with open(dot_file, 'r') as file:
        for line in tqdm(file, desc="Processing Roary nodes and edges", unit=" lines", total=total_lines):
            edge_match = edge_pattern.match(line)
            if edge_match:
                source = clean_dot_label(edge_match.group(1))
                target = clean_dot_label(edge_match.group(2))
                attributes = edge_match.group(3) or ''
                weight_match = weight_pattern.search(attributes)
                weight = float(weight_match.group(1)) if weight_match else np.nan
            ...
            if node_match:
                add_node(clean_dot_label(node_match.group(1)))

    return graph


roary_graph = roary_dot_to_igraph(roary_file_path)
roary_num_nodes = roary_graph.vcount()
roary_num_edges = roary_graph.ecount()
roary_num_subgraphs = roary_graph.decompose(mode='weak')
print("Finish query nodes and edges roary")
...
# record nodes, edge, and subgraphs of roary
roary_result = ['roary', roary_num_nodes, roary_num_edges, len(roary_num_subgraphs)]
results.append(roary_result)
...
roary_degrees = roary_graph.degree()
...
roary_degree_counts = count_nodes_by_degree(roary_degrees)
...
roary_degree_counts = dict(sorted(roary_degree_counts.items()))
...
all_degrees = set(list(panaroo_degree_counts.keys()) + list(PPanGGOLiN_degree_counts.keys()) + list(
    ...
    pangraph_degree_counts.keys()) + list(roary_degree_counts.keys()))
...
for tool, degree_counts in [('panaroo', panaroo_degree_counts), ('roary', roary_degree_counts),
                            ('PPanGGOLiN', PPanGGOLiN_degree_counts),
...
# Set tools list
Tool_list = ['panaroo', 'roary', 'PPanGGOLiN', 'ggcaller', 'bifrost', 'cuttlefish', 'pangraph']

# set color
pal = sns.color_palette(palette='coolwarm', n_colors=len(Tool_list))
...
for index, Tool in enumerate(Tool_list):
    ...
    for i in range(1, 61):
        ...
        if y_val <= len(Tool_list):  # Ensure the line is within the y-axis range
            fig.add_trace(go.Scatter(
                x=[0, max_degree],
                y=[y_val, y_val],
                mode='lines',
...
fig.update_layout(
    ...
    yaxis=dict(
        ...
        range=[0, len(Tool_list)]  # set y axis range
    ),
    plot_bgcolor='white',
    paper_bgcolor='white'
)
...
roary_df = get_nodes_and_degrees(roary_graph)
...
# merge dataframes
df = pd.concat([panaroo_df, roary_df, PPanGGOLiN_df, ggcaller_df, bifrost_df, cuttlefish_df, pangraph_df])
...
def roary_sequence_length_from_presence_absence(input_directory, roary_graph):
    candidate_files = [
        os.path.join(input_directory, 'roary_gene_presence_absence', 'gene_presence_absence.csv'),
        os.path.join(input_directory, 'abundances', 'gene_presence_absence.csv'),
        os.path.join(input_directory, 'abundances', 'roary_gene_presence_absence.csv'),
        os.path.join(input_directory, 'gene_presence_absence.csv'),
        os.path.join(input_directory, 'roary_gene_presence_absence.csv')
    ]
    roary_presence_absence_file = next((path for path in candidate_files if os.path.exists(path)), None)

    if roary_presence_absence_file is None:
        print("Warning: Roary gene_presence_absence.csv not found; Roary node lengths will be NA")
        roary_node_ids = roary_graph.vs["name"]
        roary_components = roary_graph.components(mode='weak')
        return pd.DataFrame({
            "Tool": "roary",
            "length": np.nan,
            "node_id": roary_node_ids,
            "subgraph_id": roary_components.membership
        })

    roary_presence_absence = pd.read_csv(roary_presence_absence_file, low_memory=False)
    required_columns = {'Gene', 'Avg group size nuc'}
    if not required_columns.issubset(roary_presence_absence.columns):
        print("Warning: Roary gene_presence_absence.csv is missing Gene or Avg group size nuc; Roary node lengths will be NA")
        roary_node_ids = roary_graph.vs["name"]
        roary_components = roary_graph.components(mode='weak')
        return pd.DataFrame({
            "Tool": "roary",
    ...
    roary_presence_absence['Avg group size nuc'] = pd.to_numeric(
        roary_presence_absence['Avg group size nuc'],
        errors='coerce'
    )

    roary_components = roary_graph.components(mode='weak')
    roary_subgraph_map = {
        roary_graph.vs["name"][i]: roary_components.membership[i]
        for i in range(roary_graph.vcount())
    }

    roary_sequence_length = roary_presence_absence[['Gene', 'Avg group size nuc']].copy()
    roary_sequence_length = roary_sequence_length.rename(
        columns={'Gene': 'node_id', 'Avg group size nuc': 'length'}
    )
    roary_sequence_length['Tool'] = 'roary'
    roary_sequence_length['subgraph_id'] = roary_sequence_length['node_id'].map(roary_subgraph_map)
    roary_sequence_length = roary_sequence_length[['Tool', 'length', 'node_id', 'subgraph_id']]

    matched_subgraphs = roary_sequence_length['subgraph_id'].notna().sum()
    matched_lengths = roary_sequence_length['length'].notna().sum()
    print(f"Using Roary node lengths from {roary_presence_absence_file}")
    print(f"Matched Roary node lengths: {matched_lengths}/{len(roary_sequence_length)}")
    print(f"Matched Roary subgraph IDs: {matched_subgraphs}/{len(roary_sequence_length)}")

    return roary_sequence_length


# Use Roary's Gene column as node_id and average nucleotide group size as node length.
roary_sequence_length = roary_sequence_length_from_presence_absence(input_path, roary_graph)

# merge dataframes of all tools
df_length_merge = pd.concat([
    panaroo_sequence_length,
    roary_sequence_length,
...
summary_stats_length = df_length_merge_input.groupby('Tool')['length'].agg(
    ['median', lambda x: percentile_or_nan(x, 25), lambda x: percentile_or_nan(x, 75)])
...
# Roary graph nodes do not carry sequence length, so omit them from length plots.
df_length_plot_input = df_length_merge_input.dropna(subset=['length'])

# set color scheme
coolwarm_palette = sns.color_palette(palette='coolwarm', n_colors=len(df_length_plot_input['Tool'].unique()))
...
count = 1
boxplot_data = []
for i, group in enumerate(df_length_plot_input['Tool'].unique()):
    # Subset only observation from the group
    y = df_length_plot_input[df_length_plot_input['Tool'] == group]['length']
...
# Add a legend with the name of each distinct label
ax.legend(df_length_plot_input['Tool'].unique(), fontsize=16)
...
roary_node_counts = [subgraph.vcount() for subgraph in roary_num_subgraphs]
...
roary_node_counts_df = pd.DataFrame(roary_node_counts, columns=['No. of nodes'])
roary_node_counts_df['Tool'] = 'roary'

subgraph_df = pd.concat(
    [panaroo_node_counts_df, roary_node_counts_df, PPanGGOLiN_node_counts_df, ggcaller_node_counts_df,
     bifrost_node_counts_df, cuttlefish_node_counts_df, pangraph_node_counts_df])
